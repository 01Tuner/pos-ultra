...
/**
 * Sales Order Filters Store
 *
 * Manages filtering state and logic for Sales Order Management
 * Supports filtering by date range, customer, status, and products
 */
export const useSalesOrderFiltersStore = defineStore("salesOrderFilters", () => {
    // Filter state
    const dateFrom = ref("")
    const dateTo = ref("")
    const customer = ref("")
    ...
    function saveCurrentFilters(name) {
        ...
        localStorage.setItem(
            "pos_sales_order_filters", // Changed key
    ...
    function loadSavedFiltersFromStorage() {
        try {
            const saved = localStorage.getItem("pos_sales_order_filters") // Changed key
            if (saved) {
