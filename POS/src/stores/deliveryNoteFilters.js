...
/**
 * Delivery Note Filters Store
 *
 * Manages filtering state and logic for Delivery Note Management
 * Supports filtering by date range, customer, status, and products
 */
export const useDeliveryNoteFiltersStore = defineStore("deliveryNoteFilters", () => {
    // Filter state
    const dateFrom = ref("")
    const dateTo = ref("")
    const customer = ref("")
    ...
    function loadSavedFiltersFromStorage() {
        try {
            const saved = localStorage.getItem("pos_delivery_note_filters")
            if (saved) {
